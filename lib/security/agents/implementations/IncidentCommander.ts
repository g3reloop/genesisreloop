 ...
 */
export class IncidentCommander extends BaseSecurityAgent {
  constructor() {
    super({
  ...
  private calculateAnomalyScore(data: any): number {
    ...
    if (data.activeIncidents > 1) score = 0.85;
  ...
  protected async onStart(): Promise<void> {
    this.createAlert('P5', 'incident-commander-initialized',
      'IncidentCommander security agent initialized',
      { role: 'incident-commander' }
    );
  }
}
